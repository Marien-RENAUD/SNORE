...
name_fig_list = ["Observation", "PnP Prox", "Average PnP"]#, "Average PnP Prox", "PnP GD"

n = len(name_fig_list) + 2
m = len(im_name_list)
...
for i, im_name in enumerate(im_name_list):
    ...
    text_params = {'xy': (5, 5), 'text': r"PSNR$\uparrow$/BRISQUE$\downarrow$", 'color': 'white', 'fontsize': 21, 'va': 'top', 'ha': 'left'}
    ...
    for j, im in enumerate([blur, deblur_PnPProx, deblur_APnP]):
        ...
        rect_params = {'xy': (0, 0), 'width': width, 'height': height, 'linewidth': 1, 'edgecolor': 'black', 'facecolor': 'black'}
        ax.add_patch(plt.Rectangle(**rect_params))
        text_params = {'xy': (5, 5), 'text': "{:.2f}/{:.2f}".format(im[1], im[4]), 'color': 'white', 'fontsize': 21, 'va': 'top', 'ha': 'left'}
        ax.annotate(**text_params)
        ax.axis('off')
    ...
    ax.axis('on')
    ax.set_title("$F(x_k)$", fontsize=21)
